...
(function () {
	...
	jQuery.fn = jQuery.prototype = {
		...
		setArray: function setArray(elems) {
		...
		attr: function attr(name, value, type) {
			var options = name;
		...
		text: function text(_text) {
			if ((typeof _text === "undefined" ? "undefined" : _typeof(_text)) !== "object" && _text != null) return this.empty().append((this[0] && this[0].ownerDocument || document).createTextNode(_text));

			var ret = "";

			jQuery.each(_text || this, function () {
		...
		not: function not(selector) {
			if (typeof selector === "string")
				...
				if (isSimple.test(selector)) return this.pushStack(jQuery.multiFilter(selector, this, true), "not", selector);else selector = jQuery.multiFilter(selector, this);
		...
		for (; i < length; i++) {
			...
			if ((options = arguments[i]) != null)
				...
				for (var name in options) {
					...
					if (deep && copy && (typeof copy === "undefined" ? "undefined" : _typeof(copy)) === "object" && !copy.nodeType) target[name] = jQuery.extend(deep,
	...
	jQuery.extend({
		...
		prop: function prop(elem, value, type, i, name) {
		...
		className: {
			...
			add: function add(elem, classNames) {
			...
			remove: function remove(elem, classNames) {
		...
		css: function css(elem, name, force, extra) {
		...
		curCSS: function curCSS(elem, name, force) {
			var ret,
			    style = elem.style;
		...
		clean: function clean(elems, context, fragment) {
		...
		map: function map(elems, callback) {
			var ret = [];
	...
	jQuery.each({
		...
		toggleClass: function toggleClass(classNames, state) {
	...
	jQuery.extend({
		...
		data: function data(elem, name, _data) {
			...
			if (_data !== undefined) jQuery.cache[id][name] = _data;
	...
	(function () {
		...
		var _Sizzle = function Sizzle(selector, context, results, seed) {
			...
			if (parts.length > 1 && origPOS.exec(selector)) {
				...
				var ret = seed ? { expr: parts.pop(), set: makeArray(seed) } : _Sizzle.find(parts.pop(), parts.length === 1 && context.parentNode ? context.parentNode : context, isXML(context));
				set = _Sizzle.filter(ret.expr, ret.set);
			...
			if (extra) {
				_Sizzle(extra, context, results, seed);
		...
		if (document.querySelectorAll) (function () {
			...
			_Sizzle.find = oldSizzle.find;
			_Sizzle.filter = oldSizzle.filter;
			_Sizzle.selectors = oldSizzle.selectors;
			_Sizzle.matches = oldSizzle.matches;
		})();
		...
		// EXPOSE
		jQuery.find = _Sizzle;
		jQuery.filter = _Sizzle.filter;
		jQuery.expr = _Sizzle.selectors;
		jQuery.expr[":"] = jQuery.expr.filters;

		_Sizzle.selectors.filters.hidden = function (elem) {
			return elem.offsetWidth === 0 || elem.offsetHeight === 0;
		};

		_Sizzle.selectors.filters.visible = function (elem) {
			return elem.offsetWidth > 0 || elem.offsetHeight > 0;
		};

		_Sizzle.selectors.filters.animated = function (elem) {
		...
		jQuery.multiFilter = function (expr, elems, not) {
			...
			return _Sizzle.matches(expr, elems);
		};
  ...
  */
	jQuery.event = {
		...
		trigger: function trigger(event, data, elem, bubbling) {
		...
		proxy: function proxy(fn, _proxy) {
			_proxy = _proxy || function () {
				return fn.apply(this, arguments);
			};
			// Set the guid of unique handler to the same of original handler, so it can be removed
			_proxy.guid = fn.guid = fn.guid || _proxy.guid || this.guid++;
		...
		specialAll: {
			live: {
				setup: function setup(selector, namespaces) {
	...
	jQuery.fn.extend({
		bind: function bind(type, data, fn) {
			return type == "unload" ? this.one(type, data, fn) : this.each(function () {
				jQuery.event.add(this, type, fn || data, fn && data);
			});
		},

		one: function one(type, data, fn) {
	...
	jQuery.extend({
		...
		ajaxSetup: function ajaxSetup(settings) {
   ...
   */
		...
		ajax: function ajax(s) {
			...
			var onreadystatechange = function onreadystatechange(isTimeout) {
		...
		httpSuccess: function httpSuccess(xhr) {
			try {
		...
		httpNotModified: function httpNotModified(xhr, url) {
			try {
		...
		httpData: function httpData(xhr, type, s) {
	...
	jQuery.fn.extend({
		show: function show(speed, callback) {
			if (speed) {
		...
		fadeTo: function fadeTo(speed, to, callback) {
	...
	jQuery.extend({
		...
		speed: function speed(_speed, easing, fn) {
			var opt = (typeof _speed === "undefined" ? "undefined" : _typeof(_speed)) === "object" ? _speed : {
